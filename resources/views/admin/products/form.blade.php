            ...
            <div class="admin-card__body">
                ...
                <div class="danger-zone">
                    <form action="{{ route('admin.products.destroy', $product->id) }}" method="POST"
                          onsubmit="return confirm('Yakin ingin menghapus produk ini? Tidak dapat dibatalkan.')">
                        @csrf @method('DELETE')
...
document.getElementById('addSpesifikasi').addEventListener('click', () => {
    ...
    div.innerHTML = `
        <input type="text" name="spesifikasi_key[]" placeholder="Label (cth: Standar)">
        <input type="text" name="spesifikasi_val[]" placeholder="Nilai (cth: SNI 2052:2017)">
        <button type="button" class="repeater-remove" onclick="removeRepeaterItem(this)" title="Hapus">✕</button>`;
    list.appendChild(div);
});
